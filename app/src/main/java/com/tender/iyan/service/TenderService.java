...
public class TenderService {
                ...
                if (jsonObject.getString("status").equals("success")) {
                  ...
                  if (jsonArray != null) {
                    for (int i = 0; i < jsonArray.length(); i++) {
                      ...
                      String tanggal = DateFormat.format("dd", date)
                          + "-"
  ...
  public void uploadDeal(final SaveDealView view, int idRequest, int idPenawaran) {
    ...
    client.newCall(request).enqueue(new Callback() {
      @Override public void onFailure(Call call, final IOException e) {
        new Handler(Looper.getMainLooper()).post(new Runnable() {
          @Override public void run() {
            if (view != null) view.onSaveFailed(e.getMessage());
          }
        });
      }
  ...
  public void filterPenawaranByTender(final GetPenawaranView view, Tender tender, int state) {
    ...
    if (state == ALPHABET) {
      builder.setType(MultipartBody.FORM).addFormDataPart("alphabet", String.valueOf(true));
    }

    if (state == CHEAPEST) {
      builder.setType(MultipartBody.FORM).addFormDataPart("termurah", String.valueOf(true));
    }

    if (state == MOST_EXPENSIVE) {
      builder.setType(MultipartBody.FORM).addFormDataPart("termahal", String.valueOf(true));
    }
            ...
            if (response.isSuccessful()) {
              try {
                ...
                if (jsonObject.getString("status").equals("success")) {
                  List<Penawaran> penawaranList = new ArrayList<>();
                  JSONArray array = jsonObject.getJSONArray("data");
                  Log.d(TAG, "json penawaran raw : " + jsonObject.getJSONArray("data"));
                  for (int i = 0; i < array.length(); i++) {
                    JSONObject arrayValue = array.getJSONObject(i);
                    Penawaran penawaran = new Penawaran();
                    penawaran.setId(Integer.parseInt(arrayValue.getString("id")));
